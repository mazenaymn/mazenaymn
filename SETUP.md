...
### 4.5 Include Private Contributions
- Go to your GitHub profile settings and enable "Include private contributions on my profile"
- Create a personal access token with access to your account data
- Add it as a repository secret named `SNK_GITHUB_TOKEN`

### 4.6 Private Activity Graph
- The activity graph is now generated by the repository workflow from GitHub data
- To include private activity, make sure `SNK_GITHUB_TOKEN` is set to a token that can read your private contributions
- The generated SVG is published to the `output` branch and linked from the README
